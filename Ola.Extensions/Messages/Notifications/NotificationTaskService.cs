...
namespace Ola.Extensions.Messages.Notifications
{
    ...
    /// 通知清理服务。
    /// </summary>
    internal class NotificationTaskService : TaskService
    {
        ...
        private async Task<IEnumerable<Notifier>> LoadAsync()
        {
            ...
            await using (var reader = await _context.ExecuteReaderAsync($"SELECT COUNT(1) as Size, UserId FROM {_context.EntityType.Table} GROUP BY UserId;"))
            {
