...
namespace Ola.Extensions.Storages.Caching
{
    /// <summary>
    /// 存储缓存实现类。
    /// </summary>
    public class FileStorageCache : IStorageCache
    {
        ...
        public virtual string GetOrCreate(object key, IStorageCacheDependency dependency, Func<IStorageContext, string> action)
        {
            ...
            if (cache == null)
            {
                ...
                _db.Create(cache);
                OlaStorages.SaveText(GetFilePath(cacheKey, true), text, FileShare.Read);
                return text;
            }
            ...
            return OlaStorages.ReadText(path, share: FileShare.Read);
        }
        ...
        public virtual async Task<string> GetOrCreateAsync(object key, IStorageCacheDependency dependency, Func<IStorageContext, Task<string>> action)
        {
            ...
            if (cache == null)
            {
                ...
                await OlaStorages.SaveTextAsync(GetFilePath(cacheKey, true), text, FileShare.Read);
                return text;
            }
            var path = GetFilePath(cache.CacheKey);
            if (!File.Exists(path))//缓存文件不存在
            ...
            return await OlaStorages.ReadTextAsync(path, share: FileShare.Read);
        }
        ...
        public TCache GetOrCreate<TCache>(object key, IStorageCacheDependency dependency, Func<IStorageContext, TCache> action)
        {
            ...
            if (cache == null)
            {
                ...
                _db.Create(cache);
                OlaStorages.SaveText(GetFilePath(cacheKey, true), instance.ToJsonString());
                return instance;
            }
            var path = GetFilePath(cache.CacheKey);
            if (!File.Exists(path))//缓存文件不存在
            ...
            return Cores.FromJsonString<TCache>(OlaStorages.ReadText(path));
        }
        ...
        public virtual async Task<TCache> GetOrCreateAsync<TCache>(object key, IStorageCacheDependency dependency, Func<IStorageContext, Task<TCache>> action)
        {
            ...
            if (cache == null)
            {
                ...
                await _db.CreateAsync(cache);
                await OlaStorages.SaveTextAsync(GetFilePath(cacheKey, true), instance.ToJsonString());
                return instance;
            }
            var path = GetFilePath(cache.CacheKey);
            if (!File.Exists(path))//缓存文件不存在
            ...
            return Cores.FromJsonString<TCache>(await OlaStorages.ReadTextAsync(path));
        }
