...
namespace Ola
{
    ...
    public static class Cores
    {
        ...
        {
            using var md5 = new HMACMD5(GetBytes(key));
        ...
        {
            using var sha = new HMACSHA1(GetBytes(key));
        ...
        {
            using var sha = new HMACSHA256(GetBytes(key));
            var hashed = sha.ComputeHash(Encoding.UTF8.GetBytes(text));
            return hashed.ToHexString();
        }
        ...
        {
            using var sha = new HMACSHA384(GetBytes(key));
            var hashed = sha.ComputeHash(Encoding.UTF8.GetBytes(text));
            return hashed.ToHexString();
        }
        ...
        {
            using var sha = new HMACSHA512(GetBytes(key));
            var hashed = sha.ComputeHash(Encoding.UTF8.GetBytes(text));
            return hashed.ToHexString();
        }
        ...
        public static string Encrypto(string text)
        {
            ...
            {
                using var encrypto = rijndael.CreateEncryptor();
        ...
        public static IServiceProvider BuildServiceProvider(IConfiguration configuration, Action<IOlaBuilder> action = null)
        {
            var services = new ServiceCollection();
            var builder = services.AddOla(configuration);
