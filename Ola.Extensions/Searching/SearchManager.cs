...
namespace Ola.Extensions.Searching
{
    ...
    public class SearchManager : ISearchManager
    {
        ...
        public SearchManager(IDbContext<SearchIndex> indexes, IDbContext<SearchDescriptor> searches)
        {
            _indexes = indexes;
            _searches = searches;
        }
        ...
        {
            await using (var reader = await _indexes.ExecuteReaderAsync($"SELECT TOP(1) * FROM {entityType.Table} WHERE SearchIndexed = {(int)IndexedType.Pending};"))
            {
